    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            const newFiles = Array.from(e.target.files as FileList)
                .filter((f: File) => f.name.toLowerCase().endsWith('.pdf'))
                .map((file) => ({
                    file,
    ...
    const startProcessing = async () => {
        ...
        for (let i = 0; i < pendingFiles.length; i += batchSize) {
            ...
            try {
                ...
                const results = await processBatchOfPdfs(
                    batchTexts,
                    modelConfig,
                );
                ...
                setFiles((prev) =>
                    prev.map((f) => {
                        const fileResults = results.filter(
                            (r) => r.pdfName === f.file.name,
                        );
                        if (
                            fileResults.length > 0 &&
                            batch.find((bf) => bf.id === f.id)
                        ) {
                            return {
                                ...f,
                                status: 'completed',
                                ...
                                <div className="space-y-2">
                                    ...
                                    <label className="flex items-center gap-2 text-sm font-medium text-slate-600 mt-2 cursor-pointer group relative w-fit">
                                        ...
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-slate-800 text-white text-xs rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 text-center pointer-events-none">
                                            If enabled, the original PDF file
                                            will be sent directly to the model
                                            instead of extracting text locally.
                                            This requires a model that natively
                                            supports multimodal PDF reading
                                            (e.g. Gemini 1.5 Pro).
                                        </div>
                                    </label>
                                </div>
